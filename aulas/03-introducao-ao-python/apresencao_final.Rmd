...
.pull-left[
  ...
  #### `Series` com índice
  ```{python}
  series = pd.Series([1, 2, 3], [9, 'ultimo', 'tres'])
  print(series)
  ```
]
...
.center[
  Trajetória: Variação temporal de classes de LULC em uma determinada localização no espaço geográfico.
]
...
.center[
  SITS: Variação temporal de bandas espectrais em uma determinada localização no espaço geográfico.
]
...
.pull-right[
  ...
  Recuperando a linha de `posição` "1".
  
  ```{python}
  print(df.iloc[2])
  ```

  Recuperando a linha de `posição` "1" e a coluna de `posição` "2".
...
.pull-right[
  ...
  Recuperando a linha de `posição` "1" com .bold[iloc].
...
.pull-left[
  #### Carregando dados `CSV` com `read_csv`.
...
.pull-right[
  ...
  #### Salvando `DataFrames` em `CSV` com `to_csv`.
...
.pull-left[
  ...
  #### Contando os tipos de meteoritos
